...
if Yast::SuSEFirewall.class != Yast::SuSEFirewalld
  Yast::SuSEFirewall.morph_to(:fwd)
end
  ...
  describe "#FirewallD" do
    ...
    context "when verifying its basic configuration" do
      ...
      it "has its API backend set" do
        expect(subject._get_api).not_to be nil
      end
    end
    ...
    context "given a known configuration" do
      ...
      it "refuses to set an invalid service to a zone" do
        ...
        allow(subject._get_api).to receive(:get_supported_services) { ["ssh", "ntp"] }
        expect { subject.SetServices(["foobar"], ["eth0"], true) }.to raise_exception(
            Yast::SuSEFirewalServiceNotFound, /Service with name/)
      end

      it "knows about the TCP/UDP ports in a zone" do
        expect(subject.GetAdditionalServices("tcp", "external")).to eq(["999"])
      end
      ...
      it "adds new ports/protocols services in a zone" do
        ...
        expect(subject.GetAdditionalServices("tcp", "public")).to eq(["1234"])
        expect(subject.SetAdditionalServices("tcp", "public", ["1234", "5678"])).to be nil
        expect(subject.GetAdditionalServices("tcp", "public")).to eq(["1234", "5678"])
        expect(subject.SetAdditionalServices("tcp", "public", ["1234-5678"])).to be nil
        expect(subject.GetAdditionalServices("tcp", "public")).to eq(["1234-5678"])
      end
      ...
      it "can enable broadcast logging" do
        expect(subject.GetIgnoreLoggingBroadcast(nil)).to eq("no")
        expect(subject.SetIgnoreLoggingBroadcast(nil, "broadcast")).to be nil
