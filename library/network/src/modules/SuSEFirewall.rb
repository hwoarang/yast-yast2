...
module Yast
  ...
  class Firewall < Module
    ...
    #
    # @return array of symbols representing enabled backends.
    def self.get_enabled_backends
        ...
        installed_backends.each do |b|
          enabled_backends << b if Service.Enabled(@@firewall_backends[b])
        end
    ...
    def GetServicesInZones(services)
      ...
      # Do the mapping here since we may have more than one FirewallD service
      # per SF2 service
      if self.class == SuSEFirewalld
        # Rebuild the services list
        tmp_services = deep_copy(services)
        services = []
        Builtins.foreach(tmp_services) do |service|
          _sf2_to_firewalld_service(service).each do |s|
    ...
    def self.create(backend_sym = nil)
      ...
      if backend_sym == :sf2
        ...
        running_backends = self.get_running_backends

        begin
          # Only one running backend is permitted.
          raise ArgumentError if running_backends.size > 1

          # If both firewalls are enabled, then make SF2 the default one and
          # emit a warning
          if running_backends.size == 0 and enabled_backends.size > 1
  ...
  class SuSEFirewalld < Firewall
    ...
    def ReadCurrentConfiguration
      ...
      # The following may seem somewhat complicated but it is more efficient to
      # do only invoke a single firewall-cmd command instead of iterating over
      # the zones and then using all the different firewall-cmd commands to get
      # service, port, masquerade etc information from them.
      all_zone_info = @fwd_api.list_all_zones()
      # Drop empty lines
      all_zone_info.reject! { |x| x.empty? }
      ...
      all_zone_info.each do |e|
        ...
        if @@zone_attributes.any? { |w| e.include?(w.to_s) }
          ...
          if attr == :masquerade
            @SETTINGS[zone][attr] = (vals == "no" ? false : true)
          else
            vals.each { |x| @SETTINGS[zone][attr] << x }
          end
        end
      end
      ...
      begin
        ...
        GetKnownFirewallZones().each do |zone|
          ...
          if @SETTINGS[zone][:modified].include?(:masquerade)
            ...
            @SETTINGS[zone][:modified].delete(:masquerade)
          end
          ...
          if @SETTINGS[zone][:modified].include?(:interfaces)
            ...
            @SETTINGS[zone][:modified].delete(:interfaces)
          end
          ...
          if @SETTINGS[zone][:modified].include?(:services)
            ...
            @SETTINGS[zone][:modified].delete(:services)
          end
          ...
          if @SETTINGS[zone][:modified].include?(:ports)
            ...
            @SETTINGS[zone][:modified].delete(:ports)
          end
          ...
          if @SETTINGS[zone][:modified].include?(:protocols)
            ...
            @SETTINGS[zone][:modified].delete(:protocols)
          end
    ...
    def ActivateConfiguration
      ...
      # We need a running service if we want to reload the permanent
      # configuration
      unless IsStarted()
        StartServices()
      end
    ...
    # @example
    #  LANG=en_US GetZoneFullName ("external") -> "External Zone"
    #  LANG=cs_CZ GetZoneFullName ("external") -> "Externí Zóna"
    def GetZoneFullName(zone)
      zone_short_to_long = {
        "dmz" => "demilitarized"
      }

      if not @zone_names.include?(zone)
        return _("Unknown Zone")
      end
    ...
    def IsServiceSupportedInZone(service, zone)
      ...
      _sf2_to_firewalld_service(service).each do |s|
        return false if !@SETTINGS[zone][:services].include?(s)
      end
    ...
    def SetServicesForZones(services_ids, firewall_zones, new_status)
      ...
      # Rebuild services since we may have more than one FirewallD
      # service per SF2 one
      if self.class == SuSEFirewalld
      ...
      services_ids.each do |service|
        ...
        if IsStarted() and not @fwd_api.get_supported_services.include?(service)
          Builtins.y2error("Undefined service '#{service}'")
          raise(SuSEFirewalServiceNotFound,
                "Service with name '#{service}' does not exist")
        end
        zones.each do |zone|
          ...
          if new_status == true # enable
            ...
            if !@SETTINGS[zone][:services].include?(service)
            ...
            @SETTINGS[zone][:services].delete(service)
            SetModified()
            @SETTINGS[zone][:modified] << :services
          end
        end
