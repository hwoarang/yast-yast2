...
module Yast
  ...
  class SuSEFirewallServicesClass < Module
    ...
    def self.create(backend_sym = nil)
      ...
      if backend_sym == :sf2
        ...
        SuSEFirewall.morph_to(:fwd) if SuSEFirewall.is_a?(SuSEFirewalldClass)
  ...
  class SuSEFirewalldServicesClass < SuSEFirewallServicesClass
    ...
    def ReadServicesDefinedByRPMPackages
      ...
      SuSEFirewall.StartServices() unless SuSEFirewall.IsStarted()

      SuSEFirewall.api.get_services.each do |service_name|
        # Init everything
        @services[service_name] = {}
        @known_services_features.merge!(@known_metadata).each do |_, v|
          @services[service_name][v] = []
        end

        service_info = SuSEFirewall.api.info_service(service_name)
        ...
        service_attrs.each do |attr|
          next unless service_info.key?(attr)
          case attr
          when "modules"
        ...
        @services[service_name]["name"] = SuSEFirewall.api.get_service_short(service_name)
        @services[service_name]["description"] = SuSEFirewall.api.get_service_description(service_name)
    ...
    # @param Not used
    # @return	[Array<String>] of needed RPC ports
    def GetNeededRPCPorts(_service)
      []
    end
